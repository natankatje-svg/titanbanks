"""Clean B2 GPT Image 2 call — pass prompt via stdin to avoid Windows shell-quoting issues."""
import subprocess, json, urllib.request, pathlib, time
...
OUT = pathlib.Path(r"C:\Users\natan\TitanBanks\assets\06_GENERATED_OUTPUTS\ideogram\inbox\gpt_angle-02_B2_lifestyle_v4.png")
...
try:
    ...
    # Fallback: maybe it printed plain UUID + then the result later. Find result_url in stdout.
    import re
...
if url and url.startswith("http"):
    ...
    META.write_text(json.dumps({"url": url, "elapsed_s": round(elapsed, 1), "saved": str(OUT)}, indent=2))
    print(f"DOWNLOADED: {OUT} ({OUT.stat().st_size} bytes)")
else:
    print("NO URL — check stdout")
